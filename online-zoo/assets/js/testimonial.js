...
const createCard = (prop) => {
  ...
  const cardInner = `
    <div class="testimonial__header">
      <div class="testimonial__user-icon ${prop.icon}"></div>
      <div class="testimonial__wrap">
        <h3 class="testimonial__name">${prop.name}</h3>
        <span class="testimonial__from">${prop.from}</span>
        <span class="testimonial__date">${prop.date}</span>
      </div>
    </div>
    <p class="testimonial__text">${prop.text}</p>`;
...
export default () => {
  ...
  scroll.addEventListener('input', ({ target }) => {
    const { value } = target
    const cardWidth = document.querySelector('.testimonial').clientWidth;

    if(index < value) {
      items.forEach(item => {
        item.style.transform = `translateX(-${(cardWidth + 30) * value}px)`;
      });
      index = value;
    }
    if(index > value) {
